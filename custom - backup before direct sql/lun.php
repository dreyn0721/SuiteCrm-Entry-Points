...
$query = "SELECT id, name, lun, partdinthisplanyear, partdendoflastplanyear FROM {$policy->table_name}  ";
...
while ($row = $policy->db->fetchByAssoc($result)) {
        ...
        if( isset( $formula ) ){
            if( (string)$row['lun'] != (string)$formula ){ // if the current value is not the same with the new value, else we don't need to update since it's already correct
