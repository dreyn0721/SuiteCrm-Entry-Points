...
while ($row = $insured->db->fetchByAssoc($result)) {
        ...
        if( isset( $check_formula ) ){
            if( (int)$row['medicaid'] != $check_formula ){ // if the current value is not the same with the new value, else we don't need to update since it's already correct
                $record = $insured->retrieve($row['id']); //Retrieve only if necessary so we can save resource
                if( isset( $record ) && is_object( $record ) ){
                    $record->medicaid = $check_formula;
