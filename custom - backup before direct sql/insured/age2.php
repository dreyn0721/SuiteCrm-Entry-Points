...
while ($row = $insured->db->fetchByAssoc($result)) {
        ...
        if( isset( $formula ) ){
            if( (string)$row['age2'] !== (string)$formula ){ // if the current value is not the same with the new value, else we don't need to update since it's already correct
                $record = $insured->retrieve($row['id']); //Retrieve only if necessary so we can save resource
                if( isset( $record ) && is_object( $record ) ){
                    $record->age2 = $formula;


                    // echo (string)$row['age']." != ".(string)$formula."<br>";
                    // print_r( $record->save() );
                    // exit();
