...
while ($row = $policy->db->fetchByAssoc($result)) {
        ...
        if( isset( $formula ) ){
            if( (string)$row['canceldatetext'] != (string)$formula ){ // if the current value is not the same with the new value, else we don't need to update since it's already correct
                $record = $policy->retrieve($row['id']); //Retrieve only if necessary so we can save resource
                if( isset( $record ) && is_object( $record ) ){
                    $record->canceldatetext = $formula;
