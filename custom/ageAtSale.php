...
$query_insured = "SELECT id, name, dob FROM {$insured->table_name} ";
...
$query = "SELECT id, name, ageatsale, effectivedate FROM {$policy->table_name}  ";
...
while ($row = $policy->db->fetchByAssoc($result)) {
    ...
    $formula = ageAtSale( $row, $related_insured_data );

    if( isset( $formula ) && ( isset( $row['ageatsale'] ) || is_null( $row['ageatsale'] ) ) ){ //zxc
        if( strtolower( str_replace(" ", "", (string)$row['ageatsale']) ) != strtolower( str_replace(" ", "", (string)$formula ) ) ){ // if the current value is not the same with the new value, else we don't need to update since it's already correct //zxc
...
if( isset( $has_update ) && $has_update && isset( $update_data ) && is_array( $update_data ) && count( $update_data ) > 0 ){
    ...
    $update_query = "INSERT into {$policy->table_name} (id,ageatsale) VALUES ".implode( ", ", $update_data ) . " ON DUPLICATE KEY UPDATE ageatsale = VALUES(ageatsale) ";
