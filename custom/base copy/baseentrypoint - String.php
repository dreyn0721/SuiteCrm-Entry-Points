...
while ($row = $policy->db->fetchByAssoc($result)) {
    ...
    $formula = namezxc( $row, $related_insured_data );

    if( isset( $formula ) && ( isset( $row['fieldnamezxc'] ) || is_null( $row['fieldnamezxc'] ) ) ){ //zxc
        if( strtolower( str_replace(" ", "", (string)$row['fieldnamezxc']) ) != strtolower( str_replace(" ", "", (string)$formula ) ) ){ // if the current value is not the same with the new value, else we don't need to update since it's already correct //zxc
