...
$query = "SELECT id, name, nocommissionuploaded FROM {$policy->table_name}  ";
...
while ($row = $policy->db->fetchByAssoc($result)) {
    ...
    $check_formula = (int)noCommissionUploaded( $row, $related_insured_data, $commission_count_by_id );



    if( isset( $check_formula ) && ( isset( $row['nocommissionuploaded'] ) || is_null( $row['nocommissionuploaded'] ) ) ){ //zxc
        if( (int)$row['nocommissionuploaded'] != $check_formula ){ // if the current value is not the same with the new value, else we don't need to update since it's already correct
...
if( isset( $has_update ) && $has_update && isset( $update_data ) && is_array( $update_data ) && count( $update_data ) > 0 ){
    ...
    $update_query = "INSERT into {$policy->table_name} (id,nocommissionuploaded) VALUES ".implode( ", ", $update_data ) . " ON DUPLICATE KEY UPDATE nocommissionuploaded = VALUES(nocommissionuploaded) ";
