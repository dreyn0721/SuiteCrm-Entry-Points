...
$query = "SELECT id, name, isplaced, status FROM {$policy->table_name} ";
...
while ($row = $policy->db->fetchByAssoc($result)) {
    ...
    $check_formula = (int)isPlaced( $row, $related_insured_data );

    if( isset( $check_formula ) && ( isset( $row['isplaced'] ) || is_null( $row['isplaced'] ) ) ){ //zxc
        if( (int)$row['isplaced'] != $check_formula ){ // if the current value is not the same with the new value, else we don't need to update since it's already correct
...
if( isset( $has_update ) && $has_update && isset( $update_data ) && is_array( $update_data ) && count( $update_data ) > 0 ){
    ...
    $update_query = "INSERT into {$policy->table_name} (id,isplaced) VALUES ".implode( ", ", $update_data ) . " ON DUPLICATE KEY UPDATE isplaced = VALUES(isplaced) ";
