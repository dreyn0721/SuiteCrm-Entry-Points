...
$query = "SELECT id, name, hra, aetnasfpaidhra, carrier FROM {$commission->table_name} WHERE deleted != '1' ";
...
while ($row = $commission->db->fetchByAssoc($result)) {
    ...
    if( isset( $row['carrier'] ) && strtolower( str_replace(" ", "", (string)$row['carrier']) ) == "aetnasf" ){
        $check_formula = 1;
    }



    if( isset( $check_formula ) && ( isset( $row['aetnasfpaidhra'] ) || is_null( $row['aetnasfpaidhra'] ) ) ){ //zxc
        if( (int)$row['aetnasfpaidhra'] != $check_formula ){ // if the current value is not the same with the new value, else we don't need to update since it's already correct
...
if( isset( $has_update ) && $has_update && isset( $update_data ) && is_array( $update_data ) && count( $update_data ) > 0 ){
    ...
    $update_query = "INSERT into {$commission->table_name} (id,aetnasfpaidhra) VALUES ".implode( ", ", $update_data ) . " ON DUPLICATE KEY UPDATE aetnasfpaidhra = VALUES(aetnasfpaidhra) ";
