...
$query = "SELECT id, name, hra, aflacpaidhra, carrier FROM {$commission->table_name} WHERE deleted != '1' ";
...
while ($row = $commission->db->fetchByAssoc($result)) {
    ...
    if( isset( $row['carrier'] ) && strtolower( str_replace(" ", "", (string)$row['carrier']) ) == "aflac" ){
        $check_formula = 1;
    }



    if( isset( $check_formula ) && ( isset( $row['aflacpaidhra'] ) || is_null( $row['aflacpaidhra'] ) ) ){ //zxc
        if( (int)$row['aflacpaidhra'] != $check_formula ){ // if the current value is not the same with the new value, else we don't need to update since it's already correct
...
if( isset( $has_update ) && $has_update && isset( $update_data ) && is_array( $update_data ) && count( $update_data ) > 0 ){
    ...
    $update_query = "INSERT into {$commission->table_name} (id,aflacpaidhra) VALUES ".implode( ", ", $update_data ) . " ON DUPLICATE KEY UPDATE aflacpaidhra = VALUES(aflacpaidhra) ";
