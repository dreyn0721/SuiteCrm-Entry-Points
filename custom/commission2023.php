...
while ($row = $policy->db->fetchByAssoc($result)) {
    ...
    if( isset( $formula ) && ( isset( $row['commission2023'] ) || is_null( $row['commission2023'] ) ) ){ //zxc
        if( strtolower( str_replace(" ", "", (string)$row['commission2023']) ) != strtolower( str_replace(" ", "", (string)$formula ) ) ){ // if the current value is not the same with the new value, else we don't need to update since it's already correct //zxc
...
if( isset( $has_update ) && $has_update && isset( $update_data ) && is_array( $update_data ) && count( $update_data ) > 0 ){
    ...
    $update_query = "INSERT into {$policy->table_name} (id,commission2023) VALUES ".implode( ", ", $update_data ) . " ON DUPLICATE KEY UPDATE commission2023 = VALUES(commission2023) ";
