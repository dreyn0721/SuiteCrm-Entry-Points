...
$query_commission = "SELECT id, name, commission FROM {$commission->table_name} ";
...
$query = "SELECT id, name, commissions FROM {$policy->table_name}  ";
...
while ($row = $policy->db->fetchByAssoc($result)) {
    ...
    $formula = commissions( $row, $related_insured_data, $related_commission_by_id, $commission_ids_by_policy_id );

    if( isset( $formula ) && ( isset( $row['commissions'] ) || is_null( $row['commissions'] ) ) ){ //zxc
        if( strtolower( str_replace(" ", "", (string)$row['commissions']) ) != strtolower( str_replace(" ", "", (string)$formula ) ) ){ // if the current value is not the same with the new value, else we don't need to update since it's already correct //zxc
...
if( isset( $has_update ) && $has_update && isset( $update_data ) && is_array( $update_data ) && count( $update_data ) > 0 ){
    ...
    $update_query = "INSERT into {$policy->table_name} (id,commissions) VALUES ".implode( ", ", $update_data ) . " ON DUPLICATE KEY UPDATE commissions = VALUES(commissions) ";
