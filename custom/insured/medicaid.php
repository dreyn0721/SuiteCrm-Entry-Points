...
$query = "SELECT id, name, medicaid, medicaidid FROM {$insured->table_name}  ";
...
while ($row = $insured->db->fetchByAssoc($result)) {
    ...
    $check_formula = (int)insured_medicaid( $row, $related_policy_by_id, $policy_ids_by_insured_id );


    if( isset( $check_formula ) && ( isset( $row['medicaid'] ) || is_null( $row['medicaid'] ) ) ){ //zxc
        if( (int)$row['medicaid'] != $check_formula ){ // if the current value is not the same with the new value, else we don't need to update since it's already correct //zxc
...
if( isset( $has_update ) && $has_update && isset( $update_data ) && is_array( $update_data ) && count( $update_data ) > 0 ){
    ...
    $update_query = "INSERT into {$insured->table_name} (id,medicaid) VALUES " . implode( ", ", $update_data ) . " ON DUPLICATE KEY UPDATE medicaid = VALUES(medicaid) ";
