...
$query = "SELECT id, name, salutation, gender FROM {$insured->table_name}  ";
...
while ($row = $insured->db->fetchByAssoc($result)) {
    ...
    $formula = insured_salutation( $row, $related_policy_by_id, $policy_ids_by_insured_id );


    if( isset( $formula ) && ( isset( $row['salutation'] ) || is_null( $row['salutation'] ) ) ){ //zxc
        if( strtolower( str_replace(" ", "", (string)$row['salutation']) ) != strtolower( str_replace(" ", "", (string)$formula ) ) ){ // if the current value is not the same with the new value, else we don't need to update since it's already correct //zxc
            //zxc
            $has_update = true;
            $update_data[] = '("'.$row['id'].'", "'.$formula.'")';
            $total_rows_updated++;// Count the total row updated

            $row['salutation'] = $formula;
            $single_data = $row;
...
if( isset( $has_update ) && $has_update && isset( $update_data ) && is_array( $update_data ) && count( $update_data ) > 0 ){
    ...
    if( count( $update_data ) == 1 ){
        if( isset( $single_data ) && is_array( $single_data ) ){
            $update_query = 'UPDATE suitecrm.'.$insured->table_name.' SET salutation = "'.$single_data['salutation'].'" where id = "'.$single_data['id'].'"';
        ...
        $update_query = "INSERT into {$insured->table_name} (id,salutation) VALUES ".implode( ", ", $update_data ) . " ON DUPLICATE KEY UPDATE salutation = VALUES(salutation) ";
        $result = $insured->db->query($update_query);
    }
