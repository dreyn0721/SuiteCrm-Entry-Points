...
while ($row = $insured->db->fetchByAssoc($result)) {
    ...
    if( isset( $formula ) && ( isset( $row['fullmailingaddress'] ) || is_null( $row['fullmailingaddress'] ) ) ){ //zxc
        if( strtolower( str_replace(" ", "", (string)$row['fullmailingaddress']) ) != strtolower( str_replace(" ", "", (string)$formula ) ) ){ // if the current value is not the same with the new value, else we don't need to update since it's already correct //zxc
...
if( isset( $has_update ) && $has_update && isset( $update_data ) && is_array( $update_data ) && count( $update_data ) > 0 ){
    ...
    $update_query = "INSERT into {$insured->table_name} (id,fullmailingaddress) VALUES ".implode( ", ", $update_data ) . " ON DUPLICATE KEY UPDATE fullmailingaddress = VALUES(fullmailingaddress) ";
