...
$query_policy = "SELECT id, name, effdatetext FROM {$policy->table_name} where deleted != '1' ";
...
$query = "SELECT id, name, combinedtexteffdatetext FROM {$insured->table_name} where deleted != '1' ";
...
while ($row = $insured->db->fetchByAssoc($result)) {
    ...
    if( isset( $policy_ids_by_insured_id[ $row['id'] ] ) && count( $policy_ids_by_insured_id[ $row['id'] ] ) > 0 ){
        //$formula = count( $policy_ids_by_insured_id[ $row['id'] ] );effdatetext
        foreach( $policy_ids_by_insured_id[ $row['id'] ] as $related_policy_id ){
            $the_policy = $related_policy_by_id[ $related_policy_id ];
            if( isset( $the_policy['effdatetext'] ) && $the_policy['effdatetext'] ){
                $to_formula[] = $the_policy['effdatetext'];
            }
    ...
    if( isset( $formula ) && ( isset( $row['combinedtexteffdatetext'] ) || is_null( $row['combinedtexteffdatetext'] ) ) && ( isset( $row['id'] ) && $row['id'] ) ){ //zxc
        if( strtolower( str_replace(" ", "", (string)$row['combinedtexteffdatetext']) ) != strtolower( str_replace(" ", "", (string)$formula ) ) ){ // if the current value is not the same with the new value, else we don't need to update since it's already correct //zxc
...
if( isset( $has_update ) && $has_update && isset( $update_data ) && is_array( $update_data ) && count( $update_data ) > 0 ){
    ...
    $update_query = "INSERT into {$insured->table_name} (id,combinedtexteffdatetext) VALUES ".implode( ", ", $update_data ) . " ON DUPLICATE KEY UPDATE combinedtexteffdatetext = VALUES(combinedtexteffdatetext) ";
