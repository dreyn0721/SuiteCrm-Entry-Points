...
$query_policy = "SELECT id, name, rewrite FROM {$policy->table_name} where deleted != '1' ";
...
$query = "SELECT id, name, numberofrewrites FROM {$insured->table_name} where deleted != '1' ";
...
while ($row = $insured->db->fetchByAssoc($result)) {
    ...
    if( isset( $formula ) && ( isset( $row['numberofrewrites'] ) || is_null( $row['numberofrewrites'] ) ) && ( isset( $row['id'] ) && $row['id'] ) ){ //zxc
        if( strtolower( str_replace(" ", "", (string)$row['numberofrewrites']) ) != strtolower( str_replace(" ", "", (string)$formula ) ) ){ // if the current value is not the same with the new value, else we don't need to update since it's already correct //zxc
...
if( isset( $has_update ) && $has_update && isset( $update_data ) && is_array( $update_data ) && count( $update_data ) > 0 ){
    ...
    $update_query = "INSERT into {$insured->table_name} (id,numberofrewrites) VALUES ".implode( ", ", $update_data ) . " ON DUPLICATE KEY UPDATE numberofrewrites = VALUES(numberofrewrites) ";
