...
$last_48_hours = date( "Y-m-d", strtotime( date( "Y-m-d H:i:s" )." - 2 days" ) )." 00:00:00";

$query_case = "SELECT id, name FROM {$case->table_name} WHERE date_modified <= '".$case_today."' AND date_modified >= '".$last_48_hours."' AND deleted != '1' ";
...
$query = "SELECT id, name, numberofcasesinlast48hours FROM {$insured->table_name} where deleted != '1' ";
...
while ($row = $insured->db->fetchByAssoc($result)) {
    ...
    if( isset( $formula ) && ( isset( $row['numberofcasesinlast48hours'] ) || is_null( $row['numberofcasesinlast48hours'] ) ) && ( isset( $row['id'] ) && $row['id'] ) ){ //zxc
        if( strtolower( str_replace(" ", "", (string)$row['numberofcasesinlast48hours']) ) != strtolower( str_replace(" ", "", (string)$formula ) ) ){ // if the current value is not the same with the new value, else we don't need to update since it's already correct //zxc
...
if( isset( $has_update ) && $has_update && isset( $update_data ) && is_array( $update_data ) && count( $update_data ) > 0 ){
    ...
    $update_query = "INSERT into {$insured->table_name} (id,numberofcasesinlast48hours) VALUES ".implode( ", ", $update_data ) . " ON DUPLICATE KEY UPDATE numberofcasesinlast48hours = VALUES(numberofcasesinlast48hours) ";
