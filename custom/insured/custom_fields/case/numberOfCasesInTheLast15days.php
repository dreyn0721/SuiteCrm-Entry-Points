...
$last_15_days = date( "Y-m-d", strtotime( date( "Y-m-d H:i:s" )." - 15 days" ) )." 00:00:00";

$query_case = "SELECT id, name FROM {$case->table_name} WHERE date_modified <= '".$case_today."' AND date_modified >= '".$last_15_days."' AND deleted != '1' ";
...
$query = "SELECT id, name, numberofcasesinlast15days FROM {$insured->table_name} where deleted != '1' ";
...
while ($row = $insured->db->fetchByAssoc($result)) {
    ...
    if( isset( $formula ) && ( isset( $row['numberofcasesinlast15days'] ) || is_null( $row['numberofcasesinlast15days'] ) ) && ( isset( $row['id'] ) && $row['id'] ) ){ //zxc
        if( strtolower( str_replace(" ", "", (string)$row['numberofcasesinlast15days']) ) != strtolower( str_replace(" ", "", (string)$formula ) ) ){ // if the current value is not the same with the new value, else we don't need to update since it's already correct //zxc
...
if( isset( $has_update ) && $has_update && isset( $update_data ) && is_array( $update_data ) && count( $update_data ) > 0 ){
    ...
    $update_query = "INSERT into {$insured->table_name} (id,numberofcasesinlast15days) VALUES ".implode( ", ", $update_data ) . " ON DUPLICATE KEY UPDATE numberofcasesinlast15days = VALUES(numberofcasesinlast15days) ";
