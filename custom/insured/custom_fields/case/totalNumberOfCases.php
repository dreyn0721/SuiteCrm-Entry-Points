...
$query_case = "SELECT id, name FROM {$case->table_name} WHERE deleted != '1'";
...
$query = "SELECT id, name, totalnumberofcases FROM {$insured->table_name} where deleted != '1' ";
...
while ($row = $insured->db->fetchByAssoc($result)) {
    ...
    if( isset( $formula ) && ( isset( $row['totalnumberofcases'] ) || is_null( $row['totalnumberofcases'] ) ) && ( isset( $row['id'] ) && $row['id'] ) ){ //zxc
        if( strtolower( str_replace(" ", "", (string)$row['totalnumberofcases']) ) != strtolower( str_replace(" ", "", (string)$formula ) ) ){ // if the current value is not the same with the new value, else we don't need to update since it's already correct //zxc
...
if( isset( $has_update ) && $has_update && isset( $update_data ) && is_array( $update_data ) && count( $update_data ) > 0 ){
    ...
    $update_query = "INSERT into {$insured->table_name} (id,totalnumberofcases) VALUES ".implode( ", ", $update_data ) . " ON DUPLICATE KEY UPDATE totalnumberofcases = VALUES(totalnumberofcases) ";
