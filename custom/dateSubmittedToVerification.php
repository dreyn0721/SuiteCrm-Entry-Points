...
$query = "SELECT id, name, datesubmittedtoverification, vertimeoverride, date_entered, manualvertimeentry FROM {$policy->table_name} ";
...
while ($row = $policy->db->fetchByAssoc($result)) {
    ...
    $formula = dateSubmittedToVerification( $row, $related_insured_data );

    if( isset( $formula ) && ( isset( $row['datesubmittedtoverification'] ) || is_null( $row['datesubmittedtoverification'] ) ) ){ //zxc
        if( strtolower( str_replace(" ", "", (string)$row['datesubmittedtoverification']) ) != strtolower( str_replace(" ", "", (string)$formula ) ) ){ // if the current value is not the same with the new value, else we don't need to update since it's already correct //zxc
            
            //zxc
            $has_update = true;
            echo $update_data[] = '("'.$row['id'].'", "'.$formula.'")';
...
if( isset( $has_update ) && $has_update && isset( $update_data ) && is_array( $update_data ) && count( $update_data ) > 0 ){
    ...
    $update_query = "INSERT into {$policy->table_name} (id,datesubmittedtoverification) VALUES ".implode( ", ", $update_data ) . " ON DUPLICATE KEY UPDATE datesubmittedtoverification = VALUES(datesubmittedtoverification) ";
