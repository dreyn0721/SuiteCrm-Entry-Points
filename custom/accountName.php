...
$query = "SELECT id, name FROM {$policy->table_name}  ";
...
if( isset( $has_update ) && $has_update && isset( $update_data ) && is_array( $update_data ) && count( $update_data ) > 0 ){
    ...
    $update_query = "INSERT into {$policy->table_name} (id,name) VALUES ".implode( ", ", $update_data ) . " ON DUPLICATE KEY UPDATE name = VALUES(name) ";
