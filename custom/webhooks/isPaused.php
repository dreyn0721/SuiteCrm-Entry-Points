            ...
            if( $get_targets ){
                ...
                if( isset( $get_targets_result['targets'] ) && is_array( $get_targets_result['targets'] ) && count( $get_targets_result['targets'] ) > 0 ){
                    foreach( $get_targets_result['targets'] as $loop_target ){
                        ...
                        if( isset( $loop_target['name'] ) && $loop_target['name'] == $user_name && isset( $loop_target['enabled'] ) ){
                            //The target we have to update
                            if( $loop_target['enabled'] === true ){
                                echo "unpaused";
                                exit();
                            } else if( $loop_target['enabled'] === false ){
                                echo "onpaused";
                                exit();
                            } else {
                                echo "Can't identify if the status is enabled/disabled";
                                exit();
                            }
                        }
                    }
