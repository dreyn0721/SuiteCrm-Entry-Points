...
$query_insured = "SELECT id, name, phone FROM {$insured->table_name} ";
...
$query = "SELECT id, name, sobfilename FROM {$policy->table_name}  ";
...
while ($row = $policy->db->fetchByAssoc($result)) {
    ...
    if( isset( $related_insured_id_by_policy_id[ $row['id'] ] ) && $related_insured_id_by_policy_id[ $row['id'] ] && isset( $related_insured_by_id[ $related_insured_id_by_policy_id[ $row['id'] ] ] ) && $related_insured_by_id[ $related_insured_id_by_policy_id[ $row['id'] ] ] ){
        ...
        //not required so we let it pass without it's related insured
    }


    $formula = sobFileName( $row, $related_insured_data );

    if( isset( $formula ) && ( isset( $row['sobfilename'] ) || is_null( $row['sobfilename'] ) ) ){ //zxc
        if( strtolower( str_replace(" ", "", (string)$row['sobfilename']) ) != strtolower( str_replace(" ", "", (string)$formula ) ) ){ // if the current value is not the same with the new value, else we don't need to update since it's already correct //zxc
...
if( isset( $has_update ) && $has_update && isset( $update_data ) && is_array( $update_data ) && count( $update_data ) > 0 ){
    ...
    $update_query = "INSERT into {$policy->table_name} (id,sobfilename) VALUES ".implode( ", ", $update_data ) . " ON DUPLICATE KEY UPDATE sobfilename = VALUES(sobfilename) ";
