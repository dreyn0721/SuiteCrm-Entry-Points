...
$query_commission = "SELECT * FROM {$commission->table_name} ";
...
$query = "SELECT id, name, paidhra FROM {$policy->table_name} ";
...
while ($row = $policy->db->fetchByAssoc($result)) {
    ...
    $check_formula = (int)paidHRA( $row, $related_insured_data, $related_commission_by_id, $commission_ids_by_policy_id );

    if( isset( $check_formula ) && ( isset( $row['paidhra'] ) || is_null( $row['paidhra'] ) ) ){ //zxc
        if( (int)$row['paidhra'] != $check_formula ){ // if the current value is not the same with the new value, else we don't need to update since it's already correct
...
if( isset( $has_update ) && $has_update && isset( $update_data ) && is_array( $update_data ) && count( $update_data ) > 0 ){
    ...
    $update_query = "INSERT into {$policy->table_name} (id,paidhra) VALUES ".implode( ", ", $update_data ) . " ON DUPLICATE KEY UPDATE paidhra = VALUES(paidhra) ";
