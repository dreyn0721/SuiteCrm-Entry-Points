...
$query = "SELECT id, name, q1count, status, customerdate, canceldate FROM {$policy->table_name} ";
...
while ($row = $policy->db->fetchByAssoc($result)) {
    ...
    $check_formula = (int)q1Count( $row, $related_insured_data );

    if( isset( $check_formula ) && ( isset( $row['q1count'] ) || is_null( $row['q1count'] ) ) ){ //zxc
        if( (int)$row['q1count'] != $check_formula ){ // if the current value is not the same with the new value, else we don't need to update since it's already correct
...
if( isset( $has_update ) && $has_update && isset( $update_data ) && is_array( $update_data ) && count( $update_data ) > 0 ){
    ...
    $update_query = "INSERT into {$policy->table_name} (id,q1count) VALUES ".implode( ", ", $update_data ) . " ON DUPLICATE KEY UPDATE q1count = VALUES(q1count) ";
