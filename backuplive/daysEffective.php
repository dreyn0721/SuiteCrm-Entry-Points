...
$query = "SELECT id, name, dayseffective, effectivedate FROM {$policy->table_name}  ";
...
while ($row = $policy->db->fetchByAssoc($result)) {
    ...
    $formula = daysEffective( $row, $related_insured_data );

    if( isset( $formula ) && ( isset( $row['dayseffective'] ) || is_null( $row['dayseffective'] ) ) ){ //zxc
        if( strtolower( str_replace(" ", "", (string)$row['dayseffective']) ) != strtolower( str_replace(" ", "", (string)$formula ) ) ){ // if the current value is not the same with the new value, else we don't need to update since it's already correct //zxc

            $has_update = true;

            $update_data[] = '("'.$row['id'].'", "'.$formula.'")'; //zxc
...
if( isset( $has_update ) && $has_update && isset( $update_data ) && is_array( $update_data ) && count( $update_data ) > 0 ){
    ...
    $update_query = "INSERT into {$policy->table_name} (id,dayseffective) VALUES ".implode( ", ", $update_data ) . " ON DUPLICATE KEY UPDATE dayseffective = VALUES(dayseffective) ";
