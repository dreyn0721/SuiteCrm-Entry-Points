...
$query_policy_insured_relationship = "SELECT * FROM `med01_insured_med01_policy_1_c` where deleted = '0' ";
...
$query = "SELECT id, name, isactive, status FROM `{$policy->table_name}`  ";
...
while ($row = $policy->db->fetchByAssoc($result)) {
    ...
    $check_formula = (int)isActive( $row, $related_insured_data );

    if( isset( $check_formula ) && ( isset( $row['isactive'] ) || is_null( $row['isactive'] ) ) ){ //zxc
        if( (int)$row['isactive'] != $check_formula ){ // if the current value is not the same with the new value, else we don't need to update since it's already correct
...
if( isset( $has_update ) && $has_update && isset( $update_data ) && is_array( $update_data ) && count( $update_data ) > 0 ){
    ...
    $update_query = "INSERT into {$policy->table_name} (id,isactive) VALUES ".implode( ", ", $update_data ) . " ON DUPLICATE KEY UPDATE isactive = VALUES(isactive) ";
