...
$query = "SELECT id, name, plantypecheck, plantype FROM {$policy->table_name} ";
...
while ($row = $policy->db->fetchByAssoc($result)) {
    ...
    $check_formula = (int)planTypeCheck( $row, $related_insured_data );

    if( isset( $check_formula ) && ( isset( $row['plantypecheck'] ) || is_null( $row['plantypecheck'] ) ) ){ //zxc
        if( (int)$row['plantypecheck'] != $check_formula ){ // if the current value is not the same with the new value, else we don't need to update since it's already correct
...
if( isset( $has_update ) && $has_update && isset( $update_data ) && is_array( $update_data ) && count( $update_data ) > 0 ){
    ...
    $update_query = "INSERT into {$policy->table_name} (id,plantypecheck) VALUES ".implode( ", ", $update_data ) . " ON DUPLICATE KEY UPDATE plantypecheck = VALUES(plantypecheck) ";
