...
$query_policy = "SELECT id, name, customerdatetext FROM {$policy->table_name} where deleted != '1' ";
...
$query = "SELECT id, name, customerdatetext FROM {$insured->table_name} where deleted != '1' ";
...
while ($row = $insured->db->fetchByAssoc($result)) {
    ...
    if( isset( $policy_ids_by_insured_id[ $row['id'] ] ) && count( $policy_ids_by_insured_id[ $row['id'] ] ) > 0 ){
        //$formula = count( $policy_ids_by_insured_id[ $row['id'] ] );customerdatetext
        foreach( $policy_ids_by_insured_id[ $row['id'] ] as $related_policy_id ){
            $the_policy = $related_policy_by_id[ $related_policy_id ];
            if( isset( $the_policy['customerdatetext'] ) && $the_policy['customerdatetext'] ){
                $to_formula[] = $the_policy['customerdatetext'];
            }
    ...
    if( isset( $formula ) && ( isset( $row['customerdatetext'] ) || is_null( $row['customerdatetext'] ) ) && ( isset( $row['id'] ) && $row['id'] ) ){ //zxc
...
if( isset( $has_update ) && $has_update && isset( $update_data ) && is_array( $update_data ) && count( $update_data ) > 0 ){
    ...
    $update_query = "INSERT into {$insured->table_name} (id,customerdatetext) VALUES ".implode( ", ", $update_data ) . " ON DUPLICATE KEY UPDATE customerdatetext = VALUES(customerdatetext) ";
