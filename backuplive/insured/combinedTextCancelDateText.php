...
$query_policy = "SELECT id, name, canceldatetext FROM {$policy->table_name} where deleted != '1' ";
...
$query = "SELECT id, name, combinedtextcanceldatetext FROM {$insured->table_name} where deleted != '1' ";
...
while ($row = $insured->db->fetchByAssoc($result)) {
    ...
    if( isset( $policy_ids_by_insured_id[ $row['id'] ] ) && count( $policy_ids_by_insured_id[ $row['id'] ] ) > 0 ){
        //$formula = count( $policy_ids_by_insured_id[ $row['id'] ] );canceldatetext
        foreach( $policy_ids_by_insured_id[ $row['id'] ] as $related_policy_id ){
            $the_policy = $related_policy_by_id[ $related_policy_id ];
            if( isset( $the_policy['canceldatetext'] ) && $the_policy['canceldatetext'] ){
                $to_formula[] = $the_policy['canceldatetext'];
            }
    ...
    if( isset( $formula ) && ( isset( $row['combinedtextcanceldatetext'] ) || is_null( $row['combinedtextcanceldatetext'] ) ) && ( isset( $row['id'] ) && $row['id'] ) ){ //zxc
        if( strtolower( str_replace(" ", "", (string)$row['combinedtextcanceldatetext']) ) != strtolower( str_replace(" ", "", (string)$formula ) ) ){ // if the current value is not the same with the new value, else we don't need to update since it's already correct //zxc
...
if( isset( $has_update ) && $has_update && isset( $update_data ) && is_array( $update_data ) && count( $update_data ) > 0 ){
    ...
    $update_query = "INSERT into {$insured->table_name} (id,combinedtextcanceldatetext) VALUES ".implode( ", ", $update_data ) . " ON DUPLICATE KEY UPDATE combinedtextcanceldatetext = VALUES(combinedtextcanceldatetext) ";
