...
$query = "SELECT id, name, numberofdeclinecampaigncases FROM {$insured->table_name} where deleted != '1' ";
...
$case_cs_issues = array(
    "Decline Follow Up",
    "Insurance Carrier Decline Follow-Up Call"
);
...
while ($row = $insured->db->fetchByAssoc($result)) {
    ...
    if( isset( $related_cases_by_insured_id[ $row['id'] ] ) && count( $related_cases_by_insured_id[ $row['id'] ] ) > 0 ){
        ...
        foreach( $related_cases_by_insured_id[ $row['id'] ] as $related_case_id ){
            ...
            if( isset( $the_case['csissue'] ) && $the_case['csissue'] ){
                if( in_array( $the_case['csissue'] , $case_cs_issues) ){
                    $formula++;
                }
            }
        }
    ...
    if( isset( $formula ) && ( isset( $row['numberofdeclinecampaigncases'] ) || is_null( $row['numberofdeclinecampaigncases'] ) ) && ( isset( $row['id'] ) && $row['id'] ) ){ //zxc
        if( strtolower( str_replace(" ", "", (string)$row['numberofdeclinecampaigncases']) ) != strtolower( str_replace(" ", "", (string)$formula ) ) ){ // if the current value is not the same with the new value, else we don't need to update since it's already correct //zxc
...
if( isset( $has_update ) && $has_update && isset( $update_data ) && is_array( $update_data ) && count( $update_data ) > 0 ){
    ...
    $update_query = "INSERT into {$insured->table_name} (id,numberofdeclinecampaigncases) VALUES ".implode( ", ", $update_data ) . " ON DUPLICATE KEY UPDATE numberofdeclinecampaigncases = VALUES(numberofdeclinecampaigncases) ";
