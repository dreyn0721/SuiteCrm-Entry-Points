...
$query_policy = "SELECT id, name, rewritefrontedto FROM {$policy->table_name} where deleted != '1' ";
...
$query = "SELECT id, name, numberofrewritefronts FROM {$insured->table_name} where deleted != '1' ";
...
while ($row = $insured->db->fetchByAssoc($result)) {
    ...
    if( isset( $policy_ids_by_insured_id[ $row['id'] ] ) && count( $policy_ids_by_insured_id[ $row['id'] ] ) > 0 ){
        //$formula = count( $policy_ids_by_insured_id[ $row['id'] ] );
        $formula = 0;
        foreach( $policy_ids_by_insured_id[ $row['id'] ] as $related_policy_id ){
            $the_policy = $related_policy_by_id[ $related_policy_id ];
            if( isset( $the_policy['rewritefrontedto'] ) && $the_policy['rewritefrontedto'] == "" && $the_policy['rewritefrontedto'] == "empty" ){
                $formula++;
            }
    ...
    if( isset( $formula ) && ( isset( $row['numberofrewritefronts'] ) || is_null( $row['numberofrewritefronts'] ) ) && ( isset( $row['id'] ) && $row['id'] ) ){ //zxc
        if( strtolower( str_replace(" ", "", (string)$row['numberofrewritefronts']) ) != strtolower( str_replace(" ", "", (string)$formula ) ) ){ // if the current value is not the same with the new value, else we don't need to update since it's already correct //zxc
...
if( isset( $has_update ) && $has_update && isset( $update_data ) && is_array( $update_data ) && count( $update_data ) > 0 ){
    ...
    $update_query = "INSERT into {$insured->table_name} (id,numberofrewritefronts) VALUES ".implode( ", ", $update_data ) . " ON DUPLICATE KEY UPDATE numberofrewritefronts = VALUES(numberofrewritefronts) ";
