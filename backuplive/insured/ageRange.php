...
$query = "SELECT id, name, agerange, dob FROM {$insured->table_name}  ";
...
while ($row = $insured->db->fetchByAssoc($result)) {
    ...
    $formula = insured_ageRange( $row, $related_policy_by_id, $policy_ids_by_insured_id );
...
if( isset( $has_update ) && $has_update && isset( $update_data ) && is_array( $update_data ) && count( $update_data ) > 0 ){
    ...
    $update_query = "INSERT into {$insured->table_name} (id,agerange) VALUES ".implode( ", ", $update_data ) . " ON DUPLICATE KEY UPDATE agerange = VALUES(agerange) ";
