...
$query = "SELECT id, name, primaryfirstname, primarylastname FROM {$insured->table_name}  ";
...
while ($row = $insured->db->fetchByAssoc($result)) {
    ...
    $formula = insured_accountName( $row, $related_policy_by_id, $policy_ids_by_insured_id );


    if( isset( $formula ) && ( isset( $row['name'] ) || is_null( $row['name'] ) ) ){ //zxc
        if( strtolower( str_replace(" ", "", (string)$row['name']) ) != strtolower( str_replace(" ", "", (string)$formula ) ) ){ // if the current value is not the same with the new value, else we don't need to update since it's already correct //zxc
...
if( isset( $has_update ) && $has_update && isset( $update_data ) && is_array( $update_data ) && count( $update_data ) > 0 ){
    ...
    $update_query = "INSERT into {$insured->table_name} (id,name) VALUES ".implode( ", ", $update_data ) . " ON DUPLICATE KEY UPDATE name = VALUES(name) ";
