...
$query = "SELECT id, name, termduration, issuedate, termdate FROM {$policy->table_name}  ";
...
while ($row = $policy->db->fetchByAssoc($result)) {
    ...
    $formula = termDuration( $row, $related_insured_data );

    if( isset( $formula ) && ( isset( $row['termduration'] ) || is_null( $row['termduration'] ) ) ){ //zxc
        if( strtolower( str_replace(" ", "", (string)$row['termduration']) ) != strtolower( str_replace(" ", "", (string)$formula ) ) ){ // if the current value is not the same with the new value, else we don't need to update since it's already correct //zxc
...
if( isset( $has_update ) && $has_update && isset( $update_data ) && is_array( $update_data ) && count( $update_data ) > 0 ){
    ...
    $update_query = "INSERT into {$policy->table_name} (id,termduration) VALUES ".implode( ", ", $update_data ) . " ON DUPLICATE KEY UPDATE termduration = VALUES(termduration) ";
